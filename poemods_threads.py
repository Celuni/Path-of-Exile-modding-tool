...
class manager(object):
  ...
  def __init__(self, actionqueue, searchqueue, viewqueue):
    ...
    # never load more bytes than viewerSizeLimit in the app's viewer
    self.viewerSizeLimit = 256 * 1024
  ...
  def actionthread(self, i):
    while self.threadskeeprunning is True :
      ...
      if vala[0]=="quit" :
        ...
        # quitting we need to write the modified indexbundle back
        with self.mylock:
          self.modg.updateindexbundle()
        for i in self.modg.bench :
          print("%5.5f" % i)
      ...
      if vala[0]=="quit" :
        ...
        if vala[1] in self.modg.fullfilelistdic :
          with open(self.modg.ggpkname, "rb") as ggpkpointerio :
            result1=self.modg.readbinarydata(vala[1], ggpkpointerio, limit=self.viewerSizeLimit)
            result2=self.modg.readggpkbinarydata(vala[1], ggpkpointerio, limit=self.viewerSizeLimit)
